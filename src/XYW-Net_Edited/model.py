...
# IMAGE → ENCODER (S1,S2,S3,S4) → DECODER (F43 → F32 → F21 → OUTPUT)
# There are 4 stages:
# S1 → shallow edges full resolution
# S2 → mid-level edges half resolution
# S3 → deeper edges quarter resolution
# S4 → deepest edges eighth resolution
...
# S3 (even bigger)
#  ↓
# S4 (deepest, largest receptive field)


# Each S-block runs three detectors:
# 1. XC = X-center / X-surround (small center–surround)
# 2. YC = Y-center / Y-surround (large center–surround)
# 3. W = oriented horizontal/vertical detectors

# x → pooling
#    ↓
# XYW_S   (initializes xc, yc, w)
# XYW     (updates xc, yc, w)
# XYW_E   (merges xc + yc + w)
# shortcut (1×1 conv)
# final output = XYW_E + shortcut



# each S does the same thing:
# Split the signal into 3 pathways (xc, yc, w)
# Process each pathway
# Merge them into 1 feature map
# Add a shortcut (identity mapping)


# After passing through S1→S2→S3→S4, we get four feature maps:
# S1_output, S2_output, S3_output, S4_output
# These represent edges at four scales.



# Now the decoder receives:
# S1 (highest resolution)
# S2
# S3
# S4 (lowest resolution)
# And must merge them back to the S1 resolution.
# This is where F43 / F32 / F21 come in.
# F43 means: FUSE S4 with S3
# F32 means: FUSE S3 with S2
# F21 means: FUSE S2 with S1
class Net(nn.Module):
    def __init__(self):
...
# | ----- | --------- | ----------------- | -------------------------------- |
# | XYW_S | x         | xc, yc, w         | Start the three branches         |
# | XYW   | xc, yc, w | updated xc, yc, w | Continue processing the branches |
# | XYW_E | xc, yc, w | xc + yc + w       | Merge and end the stage          |
# XYW_S: splits the input into three feature pathways.
# XYW: define the three pathways separately.
# XYW_E: recombines the three pathways into one output.


# XC = center vs local surround (small edges)
# YC = center vs large surround (large edges)
# W detects horizontal and vertical edges separately, using depthwise convolutions (1×3 and 3×1).
...
# The decoder must:
# Upsample deep features (S4, S3, S2) back to the full resolution of S1.
# Fuse the information from deep layers into shallow layers.
# Reduce channels until we get a single-channel edge map.
